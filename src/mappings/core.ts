...
export function handleTransfer(event: Transfer): void {
  ...
  let factory = PlasmaswapFactory.load(FACTORY_ADDRESS)
...
export function handleSync(event: Sync): void {
  ...
  let plasmaswap = PlasmaswapFactory.load(FACTORY_ADDRESS)

  // reset factory liquidity by subtracting onluy tarcked liquidity
  plasmaswap.totalLiquidityETH = plasmaswap.totalLiquidityETH.minus(pair.trackedReserveETH as BigDecimal)
  ...
  plasmaswap.totalLiquidityETH = plasmaswap.totalLiquidityETH.plus(trackedLiquidityETH)
  plasmaswap.totalLiquidityUSD = plasmaswap.totalLiquidityETH.times(bundle.ethPrice)
...
export function handleMint(event: Mint): void {
  ...
  plasmaswap.txCount = plasmaswap.txCount.plus(ONE_BI)
...
export function handleSwap(event: Swap): void {
  ...
  let plasmaswap = PlasmaswapFactory.load(FACTORY_ADDRESS)
  plasmaswap.totalVolumeUSD = plasmaswap.totalVolumeUSD.plus(trackedAmountUSD)
  plasmaswap.totalVolumeETH = plasmaswap.totalVolumeETH.plus(trackedAmountETH)
  plasmaswap.untrackedVolumeUSD = plasmaswap.untrackedVolumeUSD.plus(derivedAmountUSD)
  plasmaswap.txCount = plasmaswap.txCount.plus(ONE_BI)
  ...
  let uniswapDayData = updatePlasmaswapDayData(event)
